...
export async function audit(H = window.HOLLOWBROOK) {
  ...
  if (p?.raycast && H.terrain?.heightAt) {
    ...
    // something that is supposed to be there. An earlier version of this check
    // omitted the filter and reported a 12.6 m "ground mismatch" that was simply
    // a rooftop: a false positive that cost a real investigation.
    //
    // PORTING: point this at your own layout's keep-out predicates, or replace
    // both with `() => false` to sample everywhere.
    const { isClearGround, insideAnyBuilding } = await import('../world/layout.js')
      .catch(() => ({ isClearGround: () => true, insideAnyBuilding: () => false }));
