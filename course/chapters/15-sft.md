...
full FT: 42s for 750 steps (0.06s/step) | loss 9.07 -> 1.19
```
...
full FT          379,200    750     42s    0.06      0.56   1.00     0.67  0.00   0.80 821.65
LoRA r=8          37,632    750     45s    0.06      0.25   0.00     0.00  0.00   0.80  22.21
one training step, evals excluded: full FT 48 ms | LoRA 52 ms
```

Two things to read off. First, "LoRA learns less and forgets less" in one row: with a tenth of the parameters and the same steps, rank-8 LoRA on nano learns the format and `count` and never gets the copy tasks (its validation loss flattens at 3.8 against 1.05 for full fine-tuning), while its Storyland perplexity is 22 instead of 822. The rank bounds how far each weight matrix can move, in both directions. Second, the cost. The wall-clock column includes the periodic evaluations, which are slow for a model that has not learned to stop, so the honest comparison is the micro-benchmark on the last line: 48 ms against 52 ms on an idle CPU, no saving at all. (On a shared CPU that line is noise: the loaded review run measured 2,947 ms against 837 ms, a spread that says more about the other jobs than about LoRA; benchmark on an idle machine before drawing the conclusion.) Frozen weights need no weight-gradient matmuls and the optimizer touches 37k numbers instead of 379k, but at TinyLM's size the activation-gradient work, which LoRA does not skip, is most of the step. LoRA's reliable win is memory (the Adam moments for 2.5M, or 70B, parameters that are never allocated); the speed-up is a bonus that only appears once the weight matmuls dominate. The lab's checks confirm `sft_train` merged the adapters back and that the result has the same parameter names as the base. The whole quick run takes 94 s on an idle 4-core CPU; the final review run, on the same CPU with a load average above 10 from other jobs, took 25 minutes (925 s and 381 s for the two trainings) and produced identical accuracies and perplexities, because the run is seeded. Wall-clock is the one column of the table that is not reproducible.
