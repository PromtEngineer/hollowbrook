...
Two things to read off. First, "LoRA learns less and forgets less" in one row: with a tenth of the parameters and the same steps, rank-8 LoRA on nano learns the format and `count` and never gets the copy tasks (its validation loss flattens at 3.8 against 1.05 for full fine-tuning), while its Storyland perplexity is 22 instead of 822. The rank bounds how far each weight matrix can move, in both directions. Second, the cost. The wall-clock column includes the periodic evaluations, which are slow for a model that has not learned to stop, so the honest comparison is the micro-benchmark on the last line: 48 ms against 52 ms, no saving at all. Frozen weights need no weight-gradient matmuls and the optimizer touches 37k numbers instead of 379k, but at TinyLM's size the activation-gradient work, which LoRA does not skip, is most of the step. LoRA's reliable win is memory (the Adam moments for 2.5M, or 70B, parameters that are never allocated); the speed-up is a bonus that only appears once the weight matmuls dominate. The lab's checks confirm `sft_train` merged the adapters back and that the result has the same parameter names as the base. The whole quick run takes 94 s on an idle 4-core CPU (12 minutes when the same CPU was shared with other jobs).
