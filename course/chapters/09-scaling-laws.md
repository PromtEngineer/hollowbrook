...
Read this as: the compute for a training run is six FLOPs per parameter per training token — two to go forward, four to go back. The lab checks this against `TinyLM.flops_per_token()`, which adds the attention-score term (12 · layers · *d* · *T* per token, quadratic in sequence length); for TinyLM at *T* = 128 that adds 23%, and for a 2026 model at *T* = 8k–128k the attention term is why FlashAttention and the sparse attention of Chapter 12 matter. A frontier run is 10<sup>25</sup>–10<sup>26</sup> FLOPs; the lab's full run is 10<sup>13</sup>.
