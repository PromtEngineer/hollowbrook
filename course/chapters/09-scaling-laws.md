...
Run `python3 labs/lab09_scaling.py` (quick: three models, 61,440 tokens each; 39 s with two threads on a shared 4-core VM, of which 7 + 9 + 17 s is training) and `--full` (1,228,800 tokens each; about 10 minutes on an idle 4-core CPU, 33 minutes when measured on the same VM under a load average of 13 — training time scales directly with how much of the machine you get).
...
train stream 357,931 tokens | budget D = 300 steps × 32 × 128 = 1,228,800 tokens per model
  ...
  m: d=160 layers=4  N=1,117,600 (+139,360 embedding)  val loss 1.007  ...


fit: L(N) = 1.005 + 1.76e+04 / N^1.150
...
6. **The attention term** adds 7–21% to 6·N·D for these models at *T* = 64–128, more for the narrow ones (attention cost is ∝ *d*·*T* per layer while parameters are ∝ *d*<sup>2</sup>). The achieved rate was 8–29 GFLOP/s in the quick run and 1.6–18 GFLOP/s in the full run (which ran while the CPU was heavily shared); an H100 delivers ~400,000 GFLOP/s at 40% MFU.
