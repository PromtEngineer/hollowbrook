...
The rows are resampled from the best checkpoint's per-item scores, so the accuracies wobble while the half-widths follow n (a quick run gave 0.30, 0.20, 0.14, 0.10, 0.05). Read the last column against the rule of thumb: at n = 100 the half-width is about 0.1, so any two checkpoints in the table whose accuracies differ by less than about 0.1 are not distinguishable by this eval, whatever the second decimal says. To resolve a 0.05 gap you need 400 items, and the labs in this course use 30–100 because each item costs a generation.
